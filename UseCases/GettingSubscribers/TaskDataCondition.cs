...
namespace UseCases.Tasks
{
    public class TaskDataCondition : BaseHandler, IHandlerGS
    {
        ...
        public new bool handle(ref JObject json, ref TaskGS task, ref string message)
        {
            ...
            Logger.Information("Check task's data, result -> " + success);
        ...
        public bool handle(TaskSubtype subtype, long sessionId, TaskData data, ref string message)
        {
            ...
            Logger.Information("Check task data. result -> " + success);
            return success;
        }
        public bool CheckUsernames(ICollection<TaskData> taskData, long sessionId, ref string message)
        {
            ...
            {
                Logger.Warning("Input taskData count is equal to 0.");
            }
            return false;
        }
        public bool CheckLocations(ICollection<TaskData> taskData, long sessionId, ref string message)
        {
            ...
            if (taskData.Count != 0)
            {
                ...
                {
                    if (!CheckExistLocation(ref session, data.dataNames, data.dataLatitute ?? 0, data.dataLongitute ?? 0, ref message))
                    {
                        return false;
                    }
                }
        ...
        public bool CheckExistHashtag(ref Session session, string hashtag, ref string message)
        {
            ...
            {
                var result = api.hashtag.GetHashtagInfo(hashtag, ref session);
                if (result.Succeeded)
                {
                    return true;
                }
        ...
        {
            var result = api.location.SearchLocation(ref session, latitute, longitute, location);
            ...
            Logger.Warning("Can't search locations, id -> " + session.sessionId);
            return null;
        }
    }
}