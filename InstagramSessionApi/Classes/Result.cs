...
namespace InstagramApiSharp.Classes
{
    public class Result<T> : IResult<T>
    {
        ...
        public bool unexceptedResponse { get; set; }
    ...
    public static class Result
    {
        ...
        public static IResult<T> UnExpectedResponse<T>(HttpResponseMessage response, string json)
        {
            ...
            {
                ResultInfo resultInfo = new ResultInfo(ResponseType.UnExpectedResponse,
                $"Unexpected response status: {response.StatusCode}");
                return new Result<T>(false, default(T), resultInfo);
            }
            else
            {
                BadStatusResponse status = ErrorHandlingHelper.GetBadStatusFromJsonString(json);
                ResponseType responseType = GetResponseType(status);
                ResultInfo resultInfo = new ResultInfo(responseType, status)
                {
        ...
        {
            ResponseType responseType = ResponseType.UnExpectedResponse;
