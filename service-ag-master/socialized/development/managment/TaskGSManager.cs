...
namespace Managment
{
    public class TaskGSManager
    {
        ...
        public TaskGSManager(Context context)
        {
            this.context = context;
        }
        public TaskGSManager()
        {
            this.context = new Context(false);
        }
        public TaskGS SelectTask(long taskId, long userId, ref string message)
        {
            TaskGS task = GetNonDelete(taskId, userId);
            if (task != null) {
                task.taskOption = context.TaskOptions.Where(o 
                    => o.taskId == task.taskId).FirstOrDefault();
                task.taskData = GetNonDeleteTaskData(task.taskId);
                task.taskFilter = context.TaskFilters.Where(f 
                    => f.taskId == task.taskId).FirstOrDefault();
                if (task.taskFilter != null) {
                    task.taskFilter.words = context.FilterWords.Where(w 
                        => w.filterId == task.taskFilter.filterId).ToList();
                }   
            }
            else
        ...
        public bool StartStopTask(long taskId, int userId, ref string message)
        {
            ...
            if (task != null) {
                if (task.taskStopped)
                    return StartTask(task, ref message);
                else
                    return StopTask(task, ref message);
            }
            else
                message = "Server can't define task."; 
            return false;
        ...
        {
            IGAccount account = GetNonDeleteSession(userId, sessionId);
            if (account != null) {
                List<TaskGS> tasks = GetTaskWithTaskData(sessionId);
                List<sbyte> types = GetTaskTypes(tasks);                        
                List<dynamic> data = SortTaskByType(tasks, types);
                return data;
            }
            else
        ...
        public List<dynamic> SortTaskByType(List<TaskGS> tasks, List<sbyte> types)
        {
            ...
            if (types != null) {
                foreach(sbyte type in types) {
                    List<dynamic> sortedTasks = AddSortedTaskByType(tasks, type);
                    data.Add(new {
                        task_type = type,
        ...
        {
            List<dynamic> outputTasks = new List<dynamic>();
            if (tasks != null) {
