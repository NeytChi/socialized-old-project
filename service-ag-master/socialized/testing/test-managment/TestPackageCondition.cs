...
{
    [TestFixture]
    public class TestPackageCondition
    {
        Context context;
        PackageCondition condition;
        public TestPackageCondition()
        {
        ...
        {
            var user = TestMockingContext.CreateUser();
        ...
        public void IGAccountsIsTrue_With_Exist_Account()
        {
            ...
            var user = TestMockingContext.CreateUser();
            condition.CreateFreeAccess(user.userId);
            TestMockingContext.CreateSession(user.userId);
        ...
        public void PostsIsTrue_With_Exist_Posts()
        {
            string message = "";
            var user = TestMockingContext.CreateUser();
            var session = TestMockingContext.CreateSession(user.userId);
            for (int i = 0; i < 5; i++)
                TestMockingContext.CreateAutoPost(session.accountId, true);

            condition.CreateFreeAccess(user.userId);
            Assert.AreEqual(condition.PostsIsTrue(user.userId, ref message), false);
            TestMockingContext.DeleteAutoPost(session.accountId);
        }
        [Test]
        ...
        public void StoriesIsTrue_With_Exist_Posts()
        {
            ...
            var session = TestMockingContext.CreateSession(user.userId);
            for (int i = 0; i < 5; i++)
                TestMockingContext.CreateAutoPost(session.accountId, false);
