...
namespace InstagramApiSharp.API.Processors
{
    ...
    public class UserProcessor
    {
        ...
        public UserProcessor(Logger log)
        {
            this.log = log;
        ...
        public IResult<InstaUser> GetUser(ref Session session, string username)
        {
            try
            {
                ...
                if (response.StatusCode != HttpStatusCode.OK)
                {
                    ...
                    result.unexceptedResponse = true;
                    log.Error("Unexpected response from UserProcessor -> GetUser. Json result ->" + json.Result);
                    return result;
                }
                ...
                if (user == null)
                {
                    ...
                    log.Warning("Can't find user by user ID ->" + username + ".");
                    return result;
                }
                if (user.Pk < 1)
                {
                    ...
                    result.unexceptedResponse = false;
                    log.Warning("User ID is incorrect ->" + username + ".");
                    return result;
                }
