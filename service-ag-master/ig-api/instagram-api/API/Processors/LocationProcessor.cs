...
namespace InstagramApiSharp.API.Processors
{
    ...
    public class LocationProcessor
    {
        ...
        public LocationProcessor(Logger log)
        {
            this.log = log;
        ...
        public IResult<InstaLocationShortList> SearchLocation(ref Session session, double latitude, double longitude, string query)
        {
            try
            {
                ...
                if (!Uri.TryCreate(uri, fields.AsQueryString(), out var newuri))
                {
                    ...
                    log.Error("Unable to create uri for location search; from LocationProcessor -> SearchLocation. id ->" 
                    + session.userId);
                    return result;
                }
                ...
                if (response.StatusCode != HttpStatusCode.OK)
                {
                    ...
                    log.Error("Unexpected response from LocationProcessor -> SearchLocation. Json result ->" 
                    + json.Result);
                    return result;
                }
                ...
                if (locations.Locations.Count <= 0)
                {
                    ...
                    result.unexceptedResponse = false;
                    log.Warning("Can't find locations by query ->" + query);
                    return result;
                }
            ...
            catch (HttpRequestException httpException)
            {
                ...
                result.unexceptedResponse = true;
                log.Error("Exception response from LocationProcessor -> SearchLocation. Message ->" 
